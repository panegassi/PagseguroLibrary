            ...
            if ($payment->getSender()->getDocuments() != null) {
                ...
                if (is_array($documents) && count($documents) == 1) {
                    foreach ($documents as $document) {
                        if (!is_null($document)) {
                                if ($document->getType() == PagSeguroDocument::getAvailableDocumentList()[0])
                                    $data['senderCPF'] = $document->getValue();
                                if ($document->getType() == PagSeguroDocument::getAvailableDocumentList()[1])
                                    $data['sendCNPJ'] = $document->getValue();
                        }
                    }
                }
            }
